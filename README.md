...
| 界面 | 访问地址 | 说明 |
|------|---------|------|
| **用户门户** | `http://localhost:8080/` | 默认入口，普通用户访问 |
| **管理后台** | `http://localhost:8080/login` | 管理员登录入口（隐藏） |

#### 用户门户 (User Portal)
- 访问 `http://localhost:8080/` 自动跳转到 `/user/login`（用户门户登录页）
- 访问 `http://localhost:8080/user/register` 进行注册
- 登录后进入用户仪表板 `/user/dashboard`
- 用户可查看节点、订阅、订单、工单等

#### 管理后台 (Admin)
- 访问 `http://localhost:8080/login` 使用管理员账号登录（注意：不是 `/user/login`）
...
1. **管理员**：访问 `http://localhost:8080/login`，使用 `admin` / `admin123` 登录
2. **普通用户**：访问 `http://localhost:8080/` 或 `/user/register` 注册账号
3. **首次登录后请立即修改密码！**
4. 建议修改 JWT 密钥（生产环境）
...
cat > agent.yaml << EOF
panel:
  address: "https://your-panel-address:8080"
