...
| **管理后台** | `http://localhost:8080/admin/` | 管理员专属后台 |

#### 用户门户 (User Portal)
- 访问 `http://localhost:8080/` 自动跳转到用户门户
- 访问 `http://localhost:8080/user/login` 进行登录
...
- 访问 `http://localhost:8080/login` 使用管理员账号登录
- 登录后进入管理仪表盘 `/admin/dashboard`
- 可管理用户、节点、套餐、订单、系统设置等
- **admin 用户访问用户门户 (http://localhost:8080) 会自动跳转到管理后台**

### ⚠️ 首次登录注意事项

1. **管理员**：访问 `http://localhost:8080/login`，使用 `admin` / `admin123` 登录，自动进入管理后台
